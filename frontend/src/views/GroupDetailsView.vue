...
const form = ref({
  ...
  date_of_birth: '',
  role: 'Student',
  student_group_id: parseInt(groupID)
})
...
onMounted(async () => {
  ...
  if (!token || !currentUser.value.role || currentUser.value.role === 'Student') {
    router.push('/welcome')
    return
  }
...
const fetchGroupDetails = async () => {
  ...
  try {
    ...
    const studentsRes = await axios.get(`${API_BASE_URL}/api/users?role=Student&student_group_id=${groupID}`)
...
const submitForm = async () => {
  ...
  try {
    ...
    if (err.response && err.response.data && err.response.data.error) {
       ...
       submitError.value = 'Failed to create student.'
    }
  }
}

const canDelete = () => {
  return currentUser.value.role === 'Admin' || currentUser.value.role === 'Supervisor' || currentUser.value.role === 'Mentor'
}
...
const executeDelete = async () => {
  ...
  try {
    ...
    alertState.value = { show: true, message: err.response?.data?.error || 'Failed to delete student' };
  } finally {
    cancelDelete()
  }
}
...
<template>
  <div class="w-full max-w-6xl">
    ...
    <div class="mb-6 flex space-x-4 items-start">
      <div class="flex-grow flex justify-between items-center">
        <div>
          <h2 class="text-3xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-indigo-600 to-purple-600 dark:from-indigo-400 dark:to-purple-400">
            {{ group ? group.school_name : 'Loading...' }}
          </h2>
          <div class="mt-2 flex items-center space-x-3">
             <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-semibold bg-indigo-100 dark:bg-indigo-900/50 text-indigo-800 dark:text-indigo-300">
               Class: {{ group ? group.class : '...' }}
             </span>
             <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-gray-100 dark:bg-slate-800 text-gray-700 dark:text-slate-300" v-if="group">
               Academic Year: {{ group.academic_year }}
             </span>
          </div>
        </div>
        <div class="flex space-x-3">
          <button @click="openModal" class="inline-flex items-center px-4 py-2 border border-transparent rounded-lg shadow-md hover:shadow-lg hover:-translate-y-0.5 transition-all text-sm font-bold text-white bg-gradient-to-r from-emerald-500 to-teal-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-emerald-500">
    ...
    <div class="bg-white dark:bg-slate-800 shadow-md overflow-hidden sm:rounded-2xl border border-slate-200 dark:border-slate-700 transition-colors duration-300">
      <div v-if="loading" class="p-8 text-center text-slate-500 dark:text-slate-400">Loading students...</div>
      <div v-else-if="error" class="p-8 text-center text-red-500 dark:text-red-400">{{ error }}</div>
      <div v-else-if="students.length === 0" class="p-8 text-center text-slate-500 dark:text-slate-400">No students found in this group.</div>
      <table v-else class="min-w-full divide-y divide-slate-200 dark:divide-slate-700">
        <thead class="bg-slate-50 dark:bg-slate-900/50 border-b border-slate-200 dark:border-slate-700 transition-colors duration-300">
          <tr>
            <th scope="col" class="px-6 py-3 text-left text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Student</th>
            ...
            <th scope="col" class="px-6 py-3 text-left text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Date of Birth</th>
        ...
        <tbody class="bg-white dark:bg-slate-800 divide-y divide-slate-100 dark:divide-slate-700/50">
          <tr v-for="u in students" :key="u.id" class="hover:bg-slate-50 dark:hover:bg-slate-700/30 transition-colors">
            <td class="px-6 py-4 whitespace-nowrap">
              <div class="flex items-center">
                <div class="flex-shrink-0 h-10 w-10 rounded-full bg-green-100 dark:bg-green-900/50 flex items-center justify-center text-green-700 dark:text-green-400 font-bold uppercase overflow-hidden">
                  <img v-if="u.avatar" :src="'https://api.dicebear.com/7.x/bottts/svg?seed=' + u.avatar" class="w-full h-full object-cover bg-green-50 dark:bg-green-900" />
                  <template v-else>{{ u.full_name ? u.full_name.charAt(0) : 'S' }}</template>
                </div>
                <div class="ml-4">
            ...
            <td class="px-6 py-4 whitespace-nowrap">
               <div class="text-sm text-gray-900 dark:text-slate-300">{{ u.date_of_birth || 'N/A' }}</div>
            </td>
            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
              <button v-if="canDelete()" @click="confirmDelete(u)" class="text-red-600 hover:text-red-900 dark:text-red-400 dark:hover:text-red-300 ml-4 font-semibold p-2">Delete</button>
            </td>
          </tr>
        </tbody>
      </table>
          ...
          <form @submit.prevent="submitForm">
            <div class="bg-white dark:bg-slate-800 px-4 pt-5 pb-4 sm:p-6 sm:pb-4 transition-colors">
              <div class="sm:flex sm:items-start">
                <div class="mx-auto flex-shrink-0 flex items-center justify-center h-12 w-12 rounded-full bg-green-100 dark:bg-green-900/50">
                  <svg class="h-6 w-6 text-green-600 dark:text-green-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 14l9-5-9-5-9 5 9 5z"></path></svg>
                </div>
                <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                  <h3 class="text-lg leading-6 font-medium text-gray-900 dark:text-white" id="modal-title">
                    Create New Student in {{ group ? group.class : 'Group' }}
                  </h3>
                  ...
                  <div class="mt-4 space-y-4">
                    ...
                    <div>
                      <label class="block text-sm font-medium text-gray-700 dark:text-slate-300">Date of Birth</label>
                      <input v-model="form.date_of_birth" type="date" required class="bg-transparent dark:text-white mt-1 block w-full border border-gray-300 dark:border-slate-600 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div class="sm:flex sm:items-start">
            ...
            <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
              ...
              <div class="mt-2 text-sm text-slate-500 dark:text-slate-400">
                Are you sure you want to permanently remove <b class="text-slate-700 dark:text-slate-300">{{ userToDelete?.full_name }}</b>? This action cannot be undone.
              </div>
            </div>
          </div>
